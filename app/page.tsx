...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen w-screen bg-[#0B0F17] text-white overflow-hidden relative font-sans">
      ...
      `}>
        ...
        {/* Profil Utilisateur */}
        {mounted && user ? (
          <div className="p-3 bg-gray-900/90 rounded-xl border border-gray-800 flex items-center gap-3 mt-4">
            {user.avatar ? (
          ...
          <Link href="/login" className="p-2.5 bg-[#7C3AED]/20 border border-[#7C3AED]/50 rounded-xl text-center text-xs font-bold text-[#06B6D4] block hover:bg-[#7C3AED]/30 transition mt-4">
            🔑 Se connecter
          </Link>
        )}
      </aside>
      ...
      {/* Conteneur de Chat */}
      <div className="flex-1 flex flex-col h-full w-full bg-[#0B0F17] relative min-w-0">
        
        {/* Header Mobile */}
        <header className="md:hidden flex items-center justify-between px-4 py-3 border-b border-gray-800/80 bg-[#070A10]/95 backdrop-blur z-20">
          <button 
        ...
        <main 
          ...
          className="flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-6 w-full custom-scrollbar"
        >
          <div className="max-w-3xl mx-auto space-y-6">
            {messages.length === 0 ? (
              <div className="min-h-[60vh] flex flex-col items-center justify-center text-center space-y-4 py-8 px-2">
                <div className="w-16 h-16 relative rounded-2xl overflow-hidden border border-gray-800 shadow-2xl mb-2">
              ...
              messages.map((msg, index) => (
                <div key={index} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {msg.role === 'assistant' && (
                    <div className="w-8 h-8 rounded-xl overflow-hidden relative border border-gray-700 flex-shrink-0 mt-1">
                      <Image src="/logo.png" alt="Penda" fill sizes="(max-width: 768px) 28px, 32px" className="object-cover" />
                    </div>
                  )}

                  <div className={`max-w-[90%] md:max-w-2xl space-y-4 ${
                    msg.role === 'user' 
                      ? 'bg-[#7C3AED] text-white p-3.5 md:p-4 rounded-2xl rounded-tr-none text-xs md:text-sm shadow-md' 
                      : 'w-full'
                  }`}>
                    {msg.type === 'text' && (
                      <div className={msg.role === 'assistant' ? 'bg-gray-900/80 p-4 rounded-2xl border border-gray-800/90 text-xs md:text-sm text-gray-200 whitespace-pre-line leading-relaxed shadow-sm' : ''}>
                        {msg.content}
                      </div>
                    )}

                    {msg.type === 'campaign' && msg.data && (
                      <div className="space-y-5 bg-gray-900/70 p-4 md:p-6 rounded-2xl border border-gray-800/90 shadow-lg">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="bg-gray-950 p-3.5 md:p-4 rounded-xl border border-gray-800/80 text-xs md:text-sm text-gray-300 whitespace-pre-line leading-relaxed">
                            {msg.data.strategy}
                          </div>
                        </div>
        ...
        {showScrollBottom && (
          <button
            ...
            className="absolute bottom-20 right-4 md:right-6 z-30 w-8 h-8 rounded-full bg-gray-900/90 border border-gray-700/80 text-gray-300 hover:text-white flex items-center justify-center shadow-lg transition duration-200 backdrop-blur-md opacity-90 hover:opacity-100 text-xs"
        ...
        <footer className="p-3 md:p-4 border-t border-gray-800/80 bg-[#070A10]/95 backdrop-blur-md w-full">
          <form onSubmit={handleSendMessage} className="max-w-3xl mx-auto flex items-center bg-gray-900/90 rounded-2xl border border-gray-800 p-1.5 md:p-2 focus-within:border-[#7C3AED] transition shadow-inner">
            <input
              type="text"
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="Ex: Vente de robes de soirée à Lubumbashi..."
              className="flex-1 bg-transparent px-3 md:px-4 py-2 outline-none text-white text-xs md:text-sm placeholder-gray-500"
            />
            <button
              type="submit"
              disabled={loading || !prompt.trim()}
              className="bg-gradient-to-r from-[#7C3AED] to-[#06B6D4] hover:opacity-90 text-white px-4 md:px-5 py-2 rounded-xl font-medium text-xs transition disabled:opacity-40"
            >
              Envoyer
            </button>
          </form>
